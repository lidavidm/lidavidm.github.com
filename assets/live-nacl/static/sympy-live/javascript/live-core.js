...
SymPy.Shell = Class.$extend({
    ...
    createNaCl: function() {
        ...
        addParam('PS_TTY_PREFIX', 'python');
        addParam('PS_TTY_RESIZE', 'tty_resize');
        addParam('PS_TTY_COLS', 80);
        addParam('PS_TTY_ROWS', 60);
    ...
    handleNaClMessage: function(e) {
        var data = e.data.slice(6);

        if (!this._ready) {
            if (data.indexOf('READY') == 0) {
                this._ready = true;
                this.outputEl.append($('<div>Python NaCl ready.</div>'));
        ...
        if (!this._handlingData && data.indexOf('OUTPUT') == 0) {
    ...
    postNaClMessage: function(message) {
        this.embed.postMessage('python' + message + '\n');
        this.embed.postMessage('pythonEND_MESSAGE\n');
    },
    ...
    render: function(el) {
        ...
        if (this.banner && !this._hasNaCl) {
            this.outputEl.append($('<div>'+this.banner+'</div>'));
        }
        else if (this._hasNaCl) {
            this.outputEl.append($('<div>Loading NaCl Python.</div>'));
        }
    ...
    evaluate: function() {
        ...
        if (!this.evaluating && !statement.match(/^\s*$/)) {
            ...
            if (this._hasNaCl) {
                this.postNaClMessage(data.statement);
            }
            else {
